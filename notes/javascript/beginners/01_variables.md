...
> **Note:** `const` with objects/arrays — the reference is constant, but contents can be mutated.
```js
const person = { name: "Alice" };
person.name = "Bob"; // ✅ mutating property, not reassigning reference
person = {};         // ❌ reassigning reference
```
...
JS moves variable/function declarations to the top of their scope during the **compilation phase** (before execution).
