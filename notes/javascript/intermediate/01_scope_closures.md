...
## Visual Overview — Scope Chain

```mermaid
%%{init: {'theme': 'base'}}%%
flowchart TB
    subgraph global["🌍 Global Scope"]
        style global fill:#e3f2fd,stroke:#1565c0,color:#000
        GV["globalVar = 'hello'"]
        subgraph func["🔷 Function Scope"]
            style func fill:#e8f5e9,stroke:#388e3c,color:#000
            FV["functionVar = 'world'"]
            subgraph block["🟠 Block Scope { }"]
                style block fill:#fff3e0,stroke:#e65100,color:#000
                BV["blockVar (let/const)"]
            end
        end
    end
    BV -. "❌ invisible outside { }" .-> FV
    FV -. "❌ invisible outside function" .-> GV
    GV -- "✅ visible everywhere" --> GV
```

## Visual Overview — How a Closure Works

```mermaid
%%{init: {'theme': 'base'}}%%
sequenceDiagram
    participant outer as outer()
    participant inner as inner() — closure
    participant memory as Memory
    outer->>memory: creates count = 0
    outer->>inner: returns inner function
    Note over outer: outer() finishes & exits
    Note over memory: count stays alive 📌
    inner->>memory: count++ → 1
    inner->>memory: count++ → 2
    inner->>memory: count++ → 3
    Note over memory: count remembered across calls!
```
