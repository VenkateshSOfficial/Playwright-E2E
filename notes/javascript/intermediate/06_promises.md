...
## Visual Overview — Promise States
...
## Visual Overview — Promise Methods Compared

```mermaid
%%{init: {'theme': 'base'}}%%
flowchart TD
    subgraph all["Promise.all() — ALL must pass"]
        style all fill:#e8f5e9,stroke:#2e7d32,color:#000
        A1["✅ P1"] & A2["✅ P2"] & A3["✅ P3"] --> AR["✅ [P1, P2, P3]"]
        A4["✅ P1"] & A5["❌ P2 fails"] & A6["✅ P3"] --> AF["❌ Rejects immediately"]
        style AF fill:#ffcdd2,stroke:#c62828
        style AR fill:#c8e6c9,stroke:#388e3c
    end
    subgraph settled["Promise.allSettled() — NEVER rejects"]
        style settled fill:#e3f2fd,stroke:#1565c0,color:#000
        S1["✅ P1"] & S2["❌ P2"] & S3["✅ P3"] --> SR["[{fulfilled},{rejected},{fulfilled}]"]
        style SR fill:#bbdefb,stroke:#1565c0
    end
    subgraph race["Promise.race() — FIRST wins"]
        style race fill:#fff3e0,stroke:#e65100,color:#000
        R1["🐢 Slow"] & R2["🐇 Fast"] --> RR["Result of Fast"]
        style RR fill:#ffe0b2,stroke:#e65100
    end
    subgraph any["Promise.any() — FIRST success wins"]
        style any fill:#fce4ec,stroke:#c62828,color:#000
        Y1["❌ P1 fails"] & Y2["✅ P2 succeeds"] & Y3["❌ P3 fails"] --> YR["✅ P2 result"]
        style YR fill:#f8bbd0,stroke:#c62828
    end
```
