...
## Visual Overview — Sequential vs Parallel

```mermaid
gantt
    title Async Execution: Sequential vs Parallel (ms)
    dateFormat  X
    axisFormat %s

    section ❌ Sequential (slow ~1500ms total)
    fetchUser     :s1, 0, 500
    fetchOrders   :s2, after s1, 500
    fetchStats    :s3, after s2, 500

    section ✅ Parallel with Promise.all (~500ms total)
    fetchUser     :p1, 0, 500
    fetchOrders   :p2, 0, 500
    fetchStats    :p3, 0, 500
```
...
%%{init: {'theme': 'base'}}%%
flowchart LR
    subgraph seq["❌ Sequential — await one at a time"]
        style seq fill:#ffebee,stroke:#c62828,color:#000
        S1["await fetchUser\n⏳ 500ms"] --> S2["await fetchOrders\n⏳ 500ms"] --> S3["await fetchStats\n⏳ 500ms"] --> ST["Total: ~1500ms"]
        style ST fill:#ffcdd2,stroke:#c62828
    end
    subgraph par["✅ Parallel — await Promise.all"]
        style par fill:#e8f5e9,stroke:#2e7d32,color:#000
        P1["fetchUser ⏳"] & P2["fetchOrders ⏳"] & P3["fetchStats ⏳"] --> PT["Total: ~500ms (all run together)"]
        style PT fill:#c8e6c9,stroke:#388e3c
    end
```
